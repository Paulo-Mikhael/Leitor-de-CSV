...
namespace LeitorDeCsv
{
	public partial class frmPlanilha : Form
	{
		...
		{
			var excelPath = planCrud.excelPath;
			var csvPath = csvCrud.csvPath;

			var config = new frmConfiguracoes();
			config.ShowDialog();

			if (excelPath != planCrud.excelPath || csvPath != csvCrud.csvPath)
			{
				this.Close();
			}
			else if (tbPlanilha.Text != planCrud.planilha)
			{
				tbPlanilha.Text = planCrud.planilha;
